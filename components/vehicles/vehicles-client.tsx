...
export function VehiclesClient({ initialVehicles, userRole }: VehiclesClientProps) {
  ...
  const parseAndFillCRLVText = (text: string) => {
    ...
    if (renavam) setValue("renavam", renavam);
    if (chassis) setValue("chassis", chassis.toUpperCase());
  ...
  const parseAndFillCRLVUrl = (urlText: string) => {
    try {
      ...
      if (plate) setValue("plate", plate.toUpperCase());
      if (renavam) setValue("renavam", renavam);
      if (chassis) setValue("chassis", chassis.toUpperCase());
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <Tabs value={activeFormTab} onValueChange={setActiveFormTab} className="w-full">
              ...
              <TabsContent value="form-basicos" className="space-y-4">
                {/* Leitor de CRLV e QR Code */}
                <div className="p-4 bg-secondary/15 rounded-lg border border-border/40 text-xs space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="font-bold text-primary uppercase flex items-center gap-1.5 text-xs">
                        <FileCheck size={14} /> Importar CRLV (Preenchimento Automático)
                      </h4>
                      <p className="text-[10px] text-muted-foreground">Arraste o PDF do CRLV ou a imagem do QR Code do documento para preencher os dados automaticamente.</p>
                    </div>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="text-[10px] text-muted-foreground bg-black/20 p-2.5 rounded border border-border/20 flex flex-col justify-center">
                      ...
                      <ul className="list-disc list-inside space-y-0.5">
                        <li><strong>PDF do CRLV:</strong> Lê o texto do documento e extrai Marca/Modelo, Placa, Renavam, Chassi, Ano e Cor.</li>
